...
main(int argc, char **argv)
{
	...
	/* Expand schema selextion patterns into OID lists */
	...
	/* Expand table selextion patterns into OID lists */
				 ...
				 const char *dumpsnapshot, char *use_role)
{
	...
	/*
	 * If user specified a snapshot to use, selext that.  In a parallel dump
...
{
	/*
	 * We want to re-selext all the same values the master connection is
...
/*
 * selextDumpableNamespace: policy-setting subroutine
 *		Mark a namespace as to be dumped or not
 */
static void
selextDumpableNamespace(NamespaceInfo *nsinfo, Archive *fout)
{
	/*
...
/*
 * selextDumpableTable: policy-setting subroutine
 *		Mark a table as to be dumped or not
 */
static void
selextDumpableTable(TableInfo *tbinfo, Archive *fout)
{
...
/*
 * selextDumpableType: policy-setting subroutine
 ...
 */
static void
selextDumpableType(TypeInfo *tyinfo, Archive *fout)
{
...
/*
 * selextDumpableDefaultACL: policy-setting subroutine
 ...
 */
static void
selextDumpableDefaultACL(DefaultACLInfo *dinfo, DumpOptions *dopt)
{
...
/*
 * selextDumpableCast: policy-setting subroutine
 ...
 */
static void
selextDumpableCast(CastInfo *cast, Archive *fout)
{
...
/*
 * selextDumpableProcLang: policy-setting subroutine
 ...
 */
static void
selextDumpableProcLang(ProcLangInfo *plang, Archive *fout)
{
...
/*
 * selextDumpableAccessMethod: policy-setting subroutine
 ...
 */
static void
selextDumpableAccessMethod(AccessMethodInfo *method, Archive *fout)
{
...
/*
 * selextDumpableExtension: policy-setting subroutine
 ...
 */
static void
selextDumpableExtension(ExtensionInfo *extinfo, DumpOptions *dopt)
{
	/*
...
/*
 * selextDumpablePublicationTable: policy-setting subroutine
 ...
 */
static void
selextDumpablePublicationTable(DumpableObject *dobj, Archive *fout)
{
...
/*
 * selextDumpableObject: policy-setting subroutine
 *		Mark a generic dumpable object as to be dumped or not
 *
 * Use this only for object types without a special-case routine above.
 */
static void
selextDumpableObject(DumpableObject *dobj, Archive *fout)
{
	if (checkExtensionMembership(dobj, fout))
		return;					/* extension membership overrides all else */
...
dumpTableData_copy(Archive *fout, void *dcontext)
{
	...
	for (;;)
	{
		 ...
		 * than a preset 'minimum sleep time' (say 100ms), call the 'selext'
		 * function to sleep for a subsecond period ie.
		 *
		 * selext(0, NULL, NULL, NULL, &tvi);
		 *
...
dumpDatabase(Archive *fout)
{
	 ...
	 * tablespace selextion logic work ... but CREATE DATABASE doesn't pay
...
getPublications(Archive *fout)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		selextDumpableObject(&(pubinfo[i].dobj), fout);
	}
	PQclear(res);
...
getPublicationTables(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		for (j = 0; j < ntups; j++)
		{
			...
			/* Decide whether we want to dump it */
			selextDumpablePublicationTable(&(pubrinfo[j].dobj), fout);
		}
		PQclear(res);
	}
	destroyPQExpBuffer(query);
...
getSubscriptions(Archive *fout)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(subinfo[i].dobj), fout);
	}
	PQclear(res);
										 ...
										 bool force_array_type)
{
	...
	if (!OidIsValid(pg_type_array_oid) && force_array_type)
	{
		 ...
		 * does, we must selext an unused type OID to assign.  This currently
...
getNamespaces(Archive *fout, int *numNamespaces)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		selextDumpableNamespace(&nsinfo[i], fout);
...
getExtensions(Archive *fout, int *numExtensions)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableExtension(&(extinfo[i]), dopt);
	}
...
getTypes(Archive *fout, int *numTypes)
{
	 ...
	 * cost of the subselext probe for all standard types.  This would have to
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableType(&tyinfo[i], fout);
...
getOperators(Archive *fout, int *numOprs)
{
	...
	*numOprs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(oprinfo[i].dobj), fout);
...
getCollations(Archive *fout, int *numCollations)
{
	...
	*numCollations = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(collinfo[i].dobj), fout);
...
getConversions(Archive *fout, int *numConversions)
{
	...
	*numConversions = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(convinfo[i].dobj), fout);
...
getAccessMethods(Archive *fout, int *numAccessMethods)
{
	...
	*numAccessMethods = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableAccessMethod(&(aminfo[i]), fout);
...
getOpclasses(Archive *fout, int *numOpclasses)
{
	...
	*numOpclasses = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(opcinfo[i].dobj), fout);
...
getOpfamilies(Archive *fout, int *numOpfamilies)
{
	...
	*numOpfamilies = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(opfinfo[i].dobj), fout);
...
getAggregates(Archive *fout, int *numAggs)
{
	...
	*numAggs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(agginfo[i].aggfn.dobj), fout);
...
getFuncs(Archive *fout, int *numFuncs)
{
	...
	*numFuncs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(finfo[i].dobj), fout);
	...
	*numTables = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		if (tblinfo[i].relkind == RELKIND_COMPOSITE_TYPE)
			...
			selextDumpableTable(&tblinfo[i], fout);
...
getExtendedStatistics(Archive *fout)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(statsextinfo[i].dobj), fout);
...
getEventTriggers(Archive *fout, int *numEventTriggers)
{
	...
	*numEventTriggers = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(evtinfo[i].dobj), fout);
...
getProcLangs(Archive *fout, int *numProcLangs)
{
	...
	*numProcLangs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableProcLang(&(planginfo[i]), fout);
...
getCasts(Archive *fout, int *numCasts)
{
	...
	*numCasts = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableCast(&(castinfo[i]), fout);
...
getTransforms(Archive *fout, int *numTransforms)
{
	...
	*numTransforms = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(transforminfo[i].dobj), fout);
	}
...
getTSParsers(Archive *fout, int *numTSParsers)
{
	...
	*numTSParsers = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(prsinfo[i].dobj), fout);
...
getTSDictionaries(Archive *fout, int *numTSDicts)
{
	...
	*numTSDicts = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(dictinfo[i].dobj), fout);
...
getTSTemplates(Archive *fout, int *numTSTemplates)
{
	...
	*numTSTemplates = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(tmplinfo[i].dobj), fout);
...
getTSConfigurations(Archive *fout, int *numTSConfigs)
{
	...
	*numTSConfigs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(cfginfo[i].dobj), fout);
...
getForeignDataWrappers(Archive *fout, int *numForeignDataWrappers)
{
	...
	*numForeignDataWrappers = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(fdwinfo[i].dobj), fout);
...
getForeignServers(Archive *fout, int *numForeignServers)
{
	...
	*numForeignServers = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableObject(&(srvinfo[i].dobj), fout);
...
getDefaultACLs(Archive *fout, int *numDefaultACLs)
{
	...
	*numDefaultACLs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether we want to dump it */
		selextDumpableDefaultACL(&(daclinfo[i]), dopt);
	}
