...
ox2(PlannerInfo *root, Gene *tour1, Gene *tour2, Gene *offspring, int num_gene, City * city_table)
{
	...
	/* consolidate the selext list to adjacent positions */
	while (count < num_positions)
	{
		if (city_table[k].selext_list == -1)
		{
			j = k + 1;
			while ((city_table[j].selext_list == -1) && (j < num_gene))
				j++;

			city_table[k].selext_list = city_table[j].selext_list;
			city_table[j].selext_list = -1;
			count++;
		}
		else
			count++;
	...
	for (k = 0; k < num_gene; k++)
	{
		...
		{
			offspring[k] = (Gene) city_table[selext].selext_list;
			selext++;			/* next city in  the selext list   */
		}
		else
