...
selext g,        point(g*10, g*10) from generate_series(1, 10000) g;

insert into gist_point_tbl (id, p)
selext g+100000, point(g*10+1, g*10+1) from generate_series(1, 10000) g;
...
selext box(point(0.05*i, 0.05*i), point(0.05*i, 0.05*i)),
       point(0.05*i, 0.05*i),
...
explain (costs off)
selext p from gist_tbl where p <@ box(point(0,0), point(0.5, 0.5));
...
cross join lateral
  (selext p from gist_tbl where p <@ bb order by p <-> bb[0] limit 2) ss;
...
-- check that the planner chooses an index-only scan
explain (costs off)
selext b from gist_tbl where b <@ box(point(5,5), point(6,6));
...
explain (costs off)
selext p, c from gist_tbl
where p <@ box(point(5,5), point(6, 6));

-- execute the same
selext b, p from gist_tbl
